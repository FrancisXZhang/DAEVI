...
class ZipReader(object):
    ...
    @staticmethod
    def npyread(path, image_name):
        # if the extension is not .npy, then turn it into .npy
        if os.path.splitext(image_name)[1] != '.npy':
            image_name = os.path.splitext(image_name)[0] + '.npy'
        zfile = ZipReader.build_file_dict(path)
        data = zfile.read(image_name)
        im = np.load(io.BytesIO(data))
        return im
# ###########################################################################
# ###########################################################################
...
class Stack(object):
    ...
    def __call__(self, img_group):
        ...
        if isinstance(img_group[0], np.ndarray):
            # Stack the images
            stacked_images = np.stack(img_group, axis=0)
            # Transpose the dimensions
            transposed_images = np.transpose(stacked_images, (3, 4, 0, 1, 2))
            # Squeeze the last dimension
            final_images = np.squeeze(transposed_images, axis=-1)
            return final_images
